...
namespace pipeline
{
    fetch::fetch(component::bus *bus, component::fifo<fetch_decode_pack_t> *fetch_decode_fifo, component::checkpoint_buffer *checkpoint_buffer, component::branch_predictor *branch_predictor, component::store_buffer *store_buffer, uint32_t init_pc)
    {
        this->bus = bus;
    ...
    /*void fetch::run(pipeline::execute::bru_feedback_pack_t bru_feedback_pack, commit_feedback_pack_t commit_feedback_pack)
    {
        if(!(commit_feedback_pack.enable && commit_feedback_pack.flush))
        {
            ...
            bool i0_has_exception = !bus->check_align(i0_pc, 4) || !bus->check_boundary(i0_pc, 4);
            uint32_t i0 = i0_has_exception ? 0 : this->bus->read32(i0_pc);
            ...
            bool i1_has_exception = !bus->check_align(i1_pc, 4) || !bus->check_boundary(i1_pc, 4);
            uint32_t i1 = i1_has_exception ? 0 : this->bus->read32(i1_pc);
            ...
            else if(!this->fetch_decode_fifo->is_full())
            {
                ...
                t_fetch_decode_pack.op_info[0].exception_id = !bus->check_align(i0_pc, 4) ? riscv_exception_t::instruction_address_misaligned : riscv_exception_t::instruction_access_fault;
                ...
                t_fetch_decode_pack.op_info[1].exception_id = !bus->check_align(i1_pc, 4) ? riscv_exception_t::instruction_address_misaligned : riscv_exception_t::instruction_access_fault;
